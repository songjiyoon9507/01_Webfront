...
function check4() {
    ...
    console.log("age:",age);
    // age에 초기값 0이 들어가있음 미입력시 값 출력 할 수 없음
    console.log(inputAge.value.length);
    // 길이가 0이라는 뜻 이걸 사용해서 미입력 상태 구분
...
function calc(op) {
    console.log(op);
    // html calc('전달인자') test 변수명 매개변수 (Parameter) op 누른대로 나옴
    // 함수 호출 시 전달되는 값을 저장하는 변수
    // 전달인자 그대로 나옴 변수명 상관 없음
