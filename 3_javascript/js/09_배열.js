...
function check1() {
    ...
    // 배열 선언 방법 확인
    const arr1 = new Array(); // new키워드 이용해서 array 배열 만듦
    const arr2 = new Array(3); // 3칸짜리 배열
    const arr3 = []; // 0칸
    const arr4 = ["사과","바나나","딸기"]; // 3칸짜리 배열
    ...
    */
    ...
    // -> 중간에 건너뛴 index는 빈칸으로 채워짐(사라지지는 않음)
    arr1[4] = "마";
    console.log("arr1 :", arr1);
}
...
function selectMenu() {
    ...
    // 저녁 메뉴로 초기화된 배열 생성 (저녁 메뉴 들어있는 배열 생성)
    const menu = ["제육볶음","돈까스","파스타","회",
                "순대국","닭갈비","초밥","김치찌개",
                "삼겹살","알탕","햄버거","피자","백반","부대찌개"];
    // menu 배열 index 범위 내에서 난수 생성
    const randomNumber = Math.floor(Math.random() * menu.length ); // menu 배열 길이만큼 랜덤 넘버
...
function check5() {
    ...
    arr[3] = arr1;
    arr[4] = arr2;
    //arr[4] = [900,800,700,600]; 만 넣어도 결과값 600 나옴
    console.log(arr);
...
function check6() {
    let count = 2;
    const arr = [];
    // 출력되는 줄 수 i i는 5줄 i=0 시작 i<=4 i++ 로 끝 arr[i]에 넣으면 됨
    // 2부터 2씩 증가하는 수 count += 2
    for (let row = 0; row < 5; row++) { // 행 제어하는 for문
        ...
        for(let col = 0; col < 5; col++) {
            ...
            count += 2; // count를 2 wmdrk
            //arr[0][0] = 2;
            //arr[0][1] = 4;
            //arr[0][2] = 6;
            //arr[0][3] = 8;
            //arr[0][4] = 10;
            // 다시 위로 올라가서 low에서 1번째 index 빈배열 5개 만듦
            // col로 들어와서 [1][0~4] 다시 count는 돌아가지 않고 계속 2씩 증가
            // count는 초기화 식이 없어서 돌아가지 않고 계속 올라감
            // col이 5되면 멈춤 
        }
    }
    console.log(arr);
