...
function classTest() {
    ...
    // divs 0, 1, 2번째 요소에 각각 접근해서 배경색 변경
...
function nameTest() {
    ...
    for(let i = 0; i < hobbyList.length; i++) {
        ...
        if(hobbyList[i].checked) { // checked 돼 있으면 true
            str += hobbyList[i].value + " "; // " " <- 띄어쓰기
            count++;
        }
    }
...
function cssTest() {
    ...
    // 이렇게까지만 써도 됨, 선택자가 선택한 요소 중 첫번째 요소만 선택됨
...
function readValue() {
    ...
    // bg.scollTop : 스크롤 윗부분 위치
    // bg.scollTop = 값 : 스크롤 윗부분을 값 위치로 이동
    // (값이 너무 크면 제일 밑으로 이동)

    bg.scrollTop = bg.scrollHeight; // 스크롤이 아래에 있음
