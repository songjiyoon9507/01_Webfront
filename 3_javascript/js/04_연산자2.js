...
function compareFn2() {
    const v1 = Number(cv3.value); // input 태그 안이라서 string
    const v2 = Number(cv4.value); // 앞에 Number() 넣어서 형 변환
    // 형변환 해줘야지 숫자 크기 비교 됨
...
// 입력된 값 만큼 누적/차감
// input4에 작성된 값만큼 count4 누적/차감
// result4에 count4에 나타나게
...
function minus4() {
    ...
    // html input에 들어온 value 값을 value로 정의
    // count4 = (count4 - value);

    count4 -= value;
    // count4에서 input에 들어온 value 값만큼 빼준다
    // 처음 시작 0 let 변수 계속 변함

    result4.innerText = count4;
    // 결과값 나타내는 창에 count4를 나타내겠다.
}
...
function checkFn5() {
    ...
    //bool6 = false bool6와 true는 같지 않음 안에() true 결과값
    //!true니까 false 결과값
}
